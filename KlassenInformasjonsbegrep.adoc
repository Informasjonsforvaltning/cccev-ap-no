...
| Multiplisitet / _Multiplicity_ | 0..1
| Kravnivå / _Requirement level_ | Valgfri / _Optional_
|Merknad / _Note_ | Bruk denne egenskapen for forretningsmessige domeneterminologier som «alder» eller «antall ansatte» og egenskapen <<Informasjonsbegrep-uttrykk-av-forventet-verdi>> til primitive datatyper som dato eller tekststreng. 

_Use this property for business domain terminology such as age or number of employees and the property "expression of the expected value" for primitive types such as a date or a string. *Norwegian extention*: restricted the usage to only business domain terminology._
|===

...
Kravet om å være i samsvar med en dokumentasjonstypeliste er oppfylt **hvis og bare hvis** den aktuelle dokumentasjonen samsvarer med **alle** typer som listen inneholder. Det er med andre ord en *`AND`* betingelse mellom typene på en gitt dokumentasjonstypeliste, og en *`OR`* betingelse mellom ulike dokumentasjonslister.
...
| Verdiområde / _Range_ | https://data.norge.no/specification/cpsv-ap-no#Dokumentasjonstype[cv:EvidenceType &#x29C9;, window="_blank", role="ext-link"]
...
| Merknad / _Note_ | Norsk utvidelse: Kravnivået endret fra valgfri til obligatorisk, og multiplisiteten fra 0..n til 1..n.

_Norwegian extension: The requirement level changed from optional to mandatory, and the multiplicity from 0..n to 1..n._
|===
